...
namespace iOS {
    ...
    FileInfo FileSystem::GetFileInfo(const std::string& path) {
        ...
        return FileInfo(safePath, type, size, modTime, isReadable, isWritable);
    }
    
    // Get the type of a file or directory
    FileType FileSystem::GetFileType(const std::string& path) { // Keep for compatibility
    ...
    std::vector<FileInfo> FileSystem::ListDirectory(const std::string& path) {
        ...
        // Check if it's a directory
        if (GetFileInfo(safePath).m_type != FileType::Directory) {
